            ...
            {categorySections.map(({ category: cat, items }, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* موبایل: نوار افقی با اسکرول و اسنپ (عیناً مثل بخش تازه‌ترین‌ها) | از sm به بعد: گرید قبلی، بدون تغییر */}
                  <div className="flex gap-4 overflow-x-auto no-scrollbar snap-x snap-mandatory scroll-smooth -mx-5 px-5 pb-1 md:-mx-8 md:px-8 sm:grid sm:grid-cols-2 sm:gap-4 sm:overflow-visible sm:snap-none sm:mx-0 sm:px-0 sm:pb-0 md:gap-6 lg:grid-cols-4">
                    {items.map((product) => (
                      <div
                        key={product.id}
                        className="w-[44%] min-w-[152px] max-w-[190px] flex-shrink-0 snap-start sm:contents"
