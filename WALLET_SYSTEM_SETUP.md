...
- [x] **تسک ۴۲:** ثبت‌نامِ یک کاربرِ تستی → رفتن به تبِ کیف‌پول → موجودی باید `$0` نشون بده.
- [x] **تسک ۴۳:** شارژِ کیف‌پول با ارزِ EUR → دیدنِ کامپوننتِ پرداختِ کریپتو → زدنِ «من پرداخت کردم» → پیامِ تلگرام باید برسه.
- [x] **تسک ۴۴:** از پنل ادمین، همون فاکتورِ شارژ رو «تایید و شارژ کن» → موجودیِ کاربر در تبِ کیف‌پول (بعدِ رفرش) باید دقیقاً معادلِ دلاریِ همون مبلغِ یورو افزایش پیدا کرده باشه.
- [x] **تسک ۴۵:** بلافاصله بعدِ تاییدِ ادمین، زنگوله‌ی نوتیف (هم در هدر دسکتاپ، هم در منوی موبایل) باید یک عدد نشون بده و با بازکردنش، پیامِ «کیف‌پولت شارژ شد» دیده بشه.
- [x] **تسک ۴۶:** روی همون فاکتورِ شارژ، دوباره «تایید و شارژ کن» رو بزن (برای شبیه‌سازیِ کلیکِ تصادفیِ دوباره) → باید پیامِ خطای «قبلاً پردازش شده» بیاد و موجودی *دوباره* اضافه نشه.
- [x] **تسک ۴۷:** یک سبدِ خرید ببند (کمتر از موجودیِ کیف‌پول) → در چک‌اوت، تبِ «پرداخت با کیف‌پول» باید فعال باشه → پرداخت رو بزن → باید فوراً (بدونِ نیاز به تاییدِ ادمین) به صفحه‌ی موفقیت بری و در پنل ادمین سفارش با وضعیتِ `paid` دیده بشه.
- [x] **تسک ۴۸:** یک سبدِ خریدِ بزرگ‌تر از موجودیِ کیف‌پول ببند → در چک‌اوت، تبِ «پرداخت با کیف‌پول» باید غیرفعال و پیامِ «موجودی کافی نیست» دیده بشه.
- [x] **تسک ۴۹:** از پنل ادمین، دکمه‌ی «تنظیم دستی» رو روی یک کاربر امتحان کن (مثلاً `-5+‬` دلار) و مطمئن شو هم موجودی درست تغییر می‌کنه، هم یک ردیفِ جدید در تاریخچه‌ی تراکنش‌های همون کاربر دیده می‌شه.
- [x] **تسک ۵۰:** مطمئن شو یک کاربرِ عادی (نه ادمین) نمی‌تونه با درخواستِ مستقیم به سوپابیس (مثلاً از کنسولِ مرورگر) مقدارِ `wallet_balance_usd` یا `is_admin` خودش رو تغییر بده (باید خطای permission بگیره — طبقِ تسکِ ۶ در فازِ ۱).

---

## 🛠 اصلاحات و رفع اشکال (پس از فاز ۹)

- [ ] **رفعِ باگ ۱:** موقعِ بازکردنِ صفحه‌ی «کیف‌پول مشتریان» در پنل ادمین، خطای `Could not find a relationship between 'wallet_topups' and 'profiles' in the schema cache` نمایش داده می‌شد.
  **علت:** ستونِ `wallet_topups.user_id` مستقیماً به `auth.users(id)` وصل بود، نه به `public.profiles(id)`؛ برای همین Supabase نمی‌تونست موقعِ کوئریِ `select('*, profiles(...)')` این دو جدول رو خودکار به هم وصل کنه.
  **راه‌حل:** قیدِ foreign key ستونِ `user_id` در جدولِ `wallet_topups` رو از `auth.users(id)` به `public.profiles(id)` تغییر دادیم (چون این دو مقدار همیشه یکی هستن، این تغییر کاملاً امن است):
  ```sql
  alter table public.wallet_topups
    drop constraint if exists wallet_topups_user_id_fkey;

  alter table public.wallet_topups
    add constraint wallet_topups_user_id_fkey
    foreign key (user_id) references public.profiles (id) on delete cascade;

  notify pgrst, 'reload schema';
  ```
  > بهادر باید این SQL رو در Supabase Dashboard → SQL Editor اجرا کنه و بعد صفحه‌ی «کیف‌پول مشتریان» رو رفرش کنه تا مطمئن بشیم خطا رفع شده.

- [ ] **رفعِ باگ ۲:** توی تبِ کیف‌پولِ پنلِ مشتری، وقتی کاربر مبلغِ شارژ رو انتخاب می‌کرد و وارد مرحله‌ی پرداخت می‌شد، هیچ راهی برای لغو و برگشتن (مثلاً برای تغییرِ مبلغ) نداشت.
  **راه‌حل:** یک دکمه‌ی «لغو و تغییرِ مبلغ» بالای کامپوننتِ `WalletTopupPayment` در `app/[locale]/profile/page.tsx` اضافه شد که کاربر رو به مرحله‌ی «انتخابِ مبلغ» برمی‌گردونه.
  > نکته‌ی فنی: فاکتورِ `pending`ی که موقعِ رسیدن به این مرحله در `wallet_topups` ساخته شده بود، با این دکمه پاک/لغو نمی‌شه (فقط در پنلِ کاربری کنار گذاشته می‌شه) — چون طبقِ RLS، کلاینت اجازه‌ی تغییرِ وضعیتِ این جدول رو نداره. این یک فاکتورِ `pending`ی بی‌ضرر و بلااستفاده در پنلِ ادمین باقی می‌مونه که هروقت خواستی می‌تونی با دکمه‌ی «لغو» (تسکِ ۳۵) پاکش کنی. اگه بعداً خواستی این‌طوری خودکار لغو بشه، بگو تا یک راهِ امن (بدونِ دورزدنِ RLS) براش بسازیم.

- [ ] **رفعِ باگ ۳:** پیامِ پایانی بعدِ ثبتِ فاکتورِ شارژ (مرحله‌ی `submitted`) به‌جای این‌که دقیقاً بگه فاکتور با چه مبلغی ثبت شده و مشتری رو به کلیک روی آیکونِ واتساپ هدایت کنه، یک متنِ کلی و مبهم نشون می‌داد.
  **راه‌حل:** متنِ کلیدهای `Wallet.submitted_title` و `Wallet.submitted_desc` در `fa.json`/`en.json` عوض شد؛ الان دقیقاً می‌گه: «فاکتورِ شما جهتِ شارژِ کیف‌پول به مبلغِ {amount} در سیستم ثبت شد. برای دریافتِ آدرسِ کیف‌پول (ولت) و ادامه‌ی پرداخت، لطفاً روی آیکونِ واتساپِ پایینِ صفحه کلیک کنید.» و در `app/[locale]/profile/page.tsx`، مبلغ و ارزِ انتخابیِ کاربر (`chargeAmount`/`chargeCurrency`) به این پیام پاس داده می‌شه تا مبلغِ دقیق نمایش داده بشه.
  > بهادر باید فایل‌های `fa.json`، `en.json` و `app/[locale]/profile/page.tsx` رو با نسخه‌های جدیدی که فرستاده شد جایگزین کنه و فلوی شارژِ کیف‌پول رو یک‌بار کامل تست کنه.
